...
def Image_Scraping(query):
    ...
    while start < 60: 
        ...
        for image_info in json.loads(json_url.text)['responseData']['results']:
            ...
            print url
            try:
                image = requests.get(url,verify=False,timeout=100)
...
print 'enter url'
query = raw_input()
#list = ['shahid+kapoor', 'christial+bale', 'deepika+padukone', 'hugh jackman','robert+downey+junior','avengers','iron+man+movie','batman+movie','adam+levine', 'one_direction']
#for query in list:
